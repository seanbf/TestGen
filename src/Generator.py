...
def genTorqueSpeed(Test_Name,Export_Path,Export_Name,Export_Format,Logging_Path, DCDC_Ixxat_Id, MCU_Ixxat_Id, Dyno_Ip, Dyno_Port, Dyno_Id, DCDC_V_Target, Speed_Demands,Speed_Lim_Fwd, Speed_Lim_Rev,Torque_Demands,Torque_Time):
    ...
    Script.append("timeout = time.time() + 10")
    Script.append("while(DCDC.getOutputVoltage() < " + str(DCDC_V_Target) + "and (time.time() < timeout):")
    Script.append("\ttime.sleep(0.1)")
    Script.append("")
    Script.append("munchLoggingPath = "+str("."))
    Script.append("munchInterop = MunchInterop.getInstance()")
    Script.append("if munchInterop is not None:")
    Script.append("\tmunchInterop.initialise(munchLoggingPath, True)")
    Script.append("\ttime.sleep(0.5)")
    Script.append("\ttry:")
    Script.append("\t\tdcdcCanDevice = CANDevice.getSpecific(munchInterop, " + str(DCDC_Ixxat_Id))
    Script.append("\t\tmcuCanDevice = CANDevice.getSpecific(munchInterop, " + str(MCU_Ixxat_Id))
